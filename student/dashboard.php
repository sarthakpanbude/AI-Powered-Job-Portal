...
<body class="bg-gray-50 flex h-screen overflow-hidden">
    ...
    <div id="apply-modal" class="hidden fixed inset-0 z-50 overflow-y-auto items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden flex flex-col relative transform scale-95 transition-all duration-300" id="modal-container">
            ...
            <!-- Pre-fill resume text for ATS simulator -->
            <?php
            $resume_prefill = "Name: " . ($student['first_name'] ?? '') . " " . ($student['last_name'] ?? '') . "\n";
            $resume_prefill .= "Current ATS Score: " . ($student['resume_score'] ?? '0') . "/100\n";
            if (!empty($skills)) {
                $resume_prefill .= "Skills: " . implode(', ', $skills) . "\n";
            }
            $resume_prefill .= "Experience: " . ($student['experience_years'] ?? '0') . " years\n";
            $resume_prefill .= "Bio: " . ($student['bio'] ?? '');
            ?>

            <!-- AI ATS Score Checker & Compatibility Scanner -->
            <div class="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-3xl border border-slate-800 p-8 lg:p-10 shadow-xl relative overflow-hidden mt-8">
                <!-- Background accents -->
                ...
                <div class="relative z-10">
                    <div class="mb-8">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 mb-2">
                            <i class="fas fa-bolt mr-1 animate-pulse"></i> Interactive AI Matcher
                        </span>
                        <h3 class="text-2xl font-black tracking-tight">AI ATS Resume Compatibility Scanner</h3>
                        <p class="text-xs text-indigo-200 mt-1 max-w-2xl">
                            Instantly check how well your resume matches any specific target job. Get real-time compatibility scoring, keyword analysis, and dynamic recommendations.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        ...
                        <div class="space-y-5">
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-indigo-300 mb-2">1. Paste / Review Your Resume Text</label>
                                <textarea id="ats-resume" rows="6" class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-4 py-3 text-xs text-slate-100 placeholder-slate-500 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition" placeholder="Paste your professional experience, skills, and summary here..."><?php echo htmlspecialchars($resume_prefill); ?></textarea>
                            </div>

                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-indigo-300 mb-2">2. Paste Target Job Description</label>
                                <textarea id="ats-jd" rows="6" class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-4 py-3 text-xs text-slate-100 placeholder-slate-500 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition" placeholder="Paste the key responsibilities, required skills, and qualifications here..."></textarea>
                            </div>

                            <button type="button" id="ats-btn" onclick="runATSAnalysis()" class="w-full bg-primary hover:bg-indigo-500 text-white font-bold py-3 px-6 rounded-xl transition shadow-lg hover:shadow-indigo-500/20 inline-flex items-center justify-center gap-2 text-xs">
                                <i class="fas fa-robot"></i> Scan & Match with AI
                            </button>
                        </div>

                        <!-- Right Results Panel -->
                        <div class="bg-slate-900/60 rounded-2xl border border-slate-800 p-6 flex flex-col justify-center min-h-[380px] relative">
                            <!-- Placeholder State -->
                            <div id="ats-placeholder" class="text-center py-12">
                                <div class="w-16 h-16 bg-indigo-500/10 rounded-full flex items-center justify-center text-2xl text-indigo-400 mx-auto mb-4 border border-indigo-500/20">
                                    <i class="fas fa-radar animate-pulse"></i>
                                </div>
                                <h4 class="text-sm font-bold text-slate-200">Ready for Instant Scan</h4>
                                <p class="text-xs text-slate-400 mt-2 max-w-xs mx-auto">
                                    Your profile data is preloaded. Paste any target job description on the left and hit the scan button to see how you match up.
                                </p>
                            </div>
                            ...
                            <div id="ats-loader" class="hidden text-center py-12">
                                ...
                                <h4 class="text-sm font-bold text-slate-200">AI Compatibility Scanner Running...</h4>
                                <p class="text-xs text-slate-400 mt-2">Checking semantic similarity and skill gap densities...</p>
                            </div>

                            <!-- Results Dashboard -->
                            <div id="ats-results" class="hidden space-y-5">
                                <div class="flex flex-col sm:flex-row items-center gap-5 pb-5 border-b border-slate-800">
                                    ...
                                    <div class="relative w-24 h-24 flex-shrink-0">
                                        ...
                                        <span id="ats-percentage" class="absolute inset-0 flex items-center justify-center text-xl font-black text-white">0%</span>
                                    </div>

                                    <div class="text-center sm:text-left">
                                        <h4 id="ats-rating" class="text-lg font-bold text-emerald-500">Excellent Match!</h4>
                                        <p id="ats-rating-sub" class="text-xs text-slate-400 mt-1 leading-relaxed">Your profile aligns exceptionally well with the target role.</p>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-3.5">
                                    <div>
                                        ...
                                        <div class="w-full bg-slate-800 rounded-full h-1.5">
                                            <div id="bar-skills" class="bg-indigo-500 h-1.5 rounded-full transition-all duration-1000" style="width: 0%"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="w-full bg-slate-800 rounded-full h-1.5">
                                            <div id="bar-keywords" class="bg-emerald-500 h-1.5 rounded-full transition-all duration-1000" style="width: 0%"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="w-full bg-slate-800 rounded-full h-1.5">
                                            <div id="bar-format" class="bg-purple-500 h-1.5 rounded-full transition-all duration-1000" style="width: 0%"></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="pt-4 border-t border-slate-800">
                                    <h5 class="text-[10px] font-black uppercase text-indigo-300 tracking-wider mb-2">AI Suggestions: Add these missing keywords</h5>
            ...
            function runATSAnalysis() {
                ...
                setTimeout(() => {
                    ...
                    if (finalScore >= 80) {
                        ...
                        ratingText.className = 'text-lg font-bold text-emerald-500';
                        ...
                        ratingText.className = 'text-lg font-bold text-yellow-500';
                        ...
                        ratingText.className = 'text-lg font-bold text-red-500';
                    ...
                    if (missingKeywords.length > 0) {
                        missingKeywords.slice(0, 5).forEach(kw => {
                            ...
                            badge.className = 'inline-flex items-center px-3 py-1 text-[10px] font-semibold bg-red-950/40 text-red-300 border border-red-800/40 rounded-full';
                            badge.innerHTML = `<i class="fas fa-plus-circle mr-1 text-red-400"></i> ${kw}`;
                            keywordsContainer.appendChild(badge);
                        });
                    } else {
                        keywordsContainer.innerHTML = '<span class="text-emerald-400 text-xs font-medium"><i class="fas fa-check-circle mr-1"></i> No critical missing keywords identified! Excellent work.</span>';
                    }
